...
contract RouteProcessor2 {
  ...
  function processRouteInternal(
    ...
    while (stream.isNotEmpty()) {
      uint8 commandCode = stream.readUint8(); // route part 1
      ...
      else if (commandCode == 4) processOnePool(stream);            // HERE
  ...
  function processOnePool(uint256 stream) private {
    address token = stream.readAddress();         // route part 2
    swap(stream, address(this), token, 0);        // here
  }
  ...
  function swap(uint256 stream, address from, address tokenIn, uint256 amountIn) private {
    uint8 poolType = stream.readUint8(); // route part 3
    if (poolType == 0) swapUniV2(stream, from, tokenIn, amountIn);
    else if (poolType == 1) swapUniV3(stream, from, tokenIn, amountIn); // HERE
  ...
  function swapUniV3(uint256 stream, address from, address tokenIn, uint256 amountIn) private {
    address pool = stream.readAddress(); // route part 4
    bool zeroForOne = stream.readUint8() > 0; // route part 5
    address recipient = stream.readAddress(); // route part 6

    lastCalledPool = pool;                  // Control the 'lastCalledPool'
    IUniswapV3Pool(pool).swap(              // pool is controlled by attacker
      recipient,
      zeroForOne,
      int256(amountIn),
  ...
  function uniswapV3SwapCallback(
    ...
    // Verification check for caller of the function
